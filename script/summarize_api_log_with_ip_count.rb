...
denylog = dir + "multi-attrs.php.denied.log"
denytmp = dir + "multi-attrs.php.denied.tmp"
`grep -E "#{datetime}:[[:digit:]]{2}" #{denylog} > #{denytmp}`
denycount = `wc -l #{denytmp}`.split(" ").first
...
`echo "[#{datetime}] #{count.strip} - #{ipcount.strip} IPs - #{denycount} rejected - #{bancount.strip} bans - CPU%: #{cpu_use.strip} - MEM USE #{mem_use_g.to_s} / #{mem_tot_g.to_s} GB - mysqld CPU%: #{mysqld_cpu.strip} MEM%: #{mysqld_mem.strip} - f2bsrv CPU%: #{fail2ban_cpu.strip} MEM%: #{fail2ban_mem.strip}" >> #{outfile}`
